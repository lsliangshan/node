...
const getPathname = function (ctx, options) {
  ...
  if (prefix && prefix.length) {
    prefix.some(item => {
      if (typeof item === 'string' && pathname.indexOf(item) === 0) {
  ...
  if (suffix && suffix.length) {
    suffix.some(item => {
      if (typeof item === 'string' && pathname.endsWith(item)) {
  ...
  if (!lib.isEmpty(subdomain)) {
    if (options.subdomain_offset) {
      ls.app.subdomainOffset = options.subdomain_offset;
    }
...
module.exports = function (options) {
  ...
  ls._caches._modules = ls._caches._modules || [];
  if (options.multi_modules) {
    ls.app.once('appReady', () => {
      //过滤禁止访问的模块
      options.deny_modules = options.deny_modules || [];
      ls._caches._modules = ls._caches._modules.filter(x => options.denyModules.indexOf(x) === -1);
    });
  }

  return function (ctx, next) {
    lib.define(ctx, 'routers', ls._caches.configs.router, 1);
    if (ctx.routers) {
    ...
    if (options.multi_modules) {
      ctx = parseDefault(ctx, pathname, options, ls._caches._modules, true);
    } else {
      ctx = parseDefault(ctx, pathname, options, ls._caches._modules);
    }
    if (ctx){
      return next();
    }
